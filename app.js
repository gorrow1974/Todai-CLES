const APP_VERSION='1.0.0',QUESTION_BANK_VERSION='jcles-r1-todai-science-1',PROFILE_KEY='jcles.activeProfile.v1',PROFILE_SET_KEY='jcles.profileConfigured.v1',NAME_KEY='jcles.learnerName.v1';
const POINT_LEVELS=[
 {level:1,min:0,max:49,name:'ウォームアップ',reward:1,penalty:0},
 {level:2,min:50,max:149,name:'チャレンジ',reward:3,penalty:1},
 {level:3,min:150,max:299,name:'アドバンス',reward:5,penalty:3},
 {level:4,min:300,max:Infinity,name:'ハイリスク',reward:10,penalty:10}
];
let pendingSettingsProfile='',DATA={items:[],groups:[]},WEEKLY={weeks:[]},TODAY={days:{},default:{}},allItems=[],groups=[],currentPack=[],currentIndex=0,currentWeekId='',currentSessionId='',answerLocked=false,timer,t0=0,limit=45,hintLevel=0,hintTimes=[],activeProfile=localStorage.getItem(PROFILE_KEY)||'';
...
async function loadJsonSafe(path,fallback){
 try{
   ...
   console.warn('J-CLES load failed:',path,err);
   return fallback;
 }
}
...
function bind(){
 ...
 E('closeLogStats').onclick=closeLogStats;
 E('logStatsModal').onclick=e=>{if(e.target===E('logStatsModal'))closeLogStats()};
 document.addEventListener('keydown',e=>{if(e.key==='Escape')closeLogStats()});
...
function currentLearnerName(){return (localStorage.getItem(NAME_KEY)||'学習者').trim()||'学習者'}
function currentLearnerId(){return currentLearnerName().normalize('NFKC').toLocaleLowerCase('ja-JP')}
...
function logs(){return (CLESStorage.load().logs||[])}
function learnerLogs(){const id=currentLearnerId();return logs().filter(l=>(l.user_profile?l.user_profile==='learner':(l.mode!=='test'&&l.app_mode!=='test'))&&(!l.learner_id||l.learner_id===id))}
function pointTotal(){return Math.max(0,learnerLogs().reduce((sum,l)=>sum+Number(l.point_delta||0),0))}
function pointRule(total=pointTotal()){return POINT_LEVELS.find(r=>total>=r.min&&total<=r.max)||POINT_LEVELS[0]}
function updatePointUI(){
 const total=pointTotal(),rule=pointRule(total),isLearner=activeProfile==='learner';
 if(E('pointChip'))E('pointChip').textContent=isLearner?`${total} pt`:'テスト中';
 if(E('pointBoardTotal'))E('pointBoardTotal').textContent=`${total} pt`;
 if(E('pointLevelName'))E('pointLevelName').textContent=`LEVEL ${rule.level}｜${rule.name}`;
 if(E('pointRuleText'))E('pointRuleText').textContent=`正解 ＋${rule.reward} ／ 不正解 ${rule.penalty?`−${rule.penalty}`:'0'}`;
}
...
function renderTodayFallback(){
 const fallback={
   ...
   message:'知識量ではなく、前後の文がどんな関係にあるかを見抜こう。',
   cles_message:'最初は正確に。慣れたら30秒以内を目指す。'
 };
...
function newSessionId(kind){return `J${Date.now()}_${kind}_${currentLearnerId().replace(/[^a-z0-9ぁ-んァ-ヶ一-龠]/gi,'').slice(0,12)||'learner'}`}
function mapItems(){return Object.fromEntries(allItems.map(x=>[x.id,x]))}function startWeek(id,review){let w=WEEKLY.weeks.find(x=>x.id===id),m=mapItems(),a=w.item_ids.map(x=>m[x]).filter(Boolean);currentPack=smartSort(a,review?weakTypes():{}).slice(0,10);currentIndex=0;currentWeekId=id;currentSessionId=newSessionId(id);E('learnTitle').textContent=`${w.label}: ${w.title}`;showScreen('learnScreen');renderQuestion()}
function startReviewQueue(){currentPack=smartSort([...allItems],weakTypes()).slice(0,10);currentIndex=0;currentWeekId='review-queue';currentSessionId=newSessionId('review');E('learnTitle').textContent='弱点リトライ';showScreen('learnScreen');renderQuestion()}
function startAdaptiveNextTen(){currentPack=smartSort([...allItems],weakTypes()).slice(0,10);currentIndex=0;currentWeekId='adaptive-next';currentSessionId=newSessionId('next');E('learnTitle').textContent='次の10問';showScreen('learnScreen');renderQuestion()}
...
function renderQuestion(){
 ...
 E('questionTitle').textContent=`${it.title}｜［注目箇所］の役割は？`;
 E('sentence').innerHTML=maskChunk(it.sentence,it.chunk)+(it.jp?`<div class="jp">${escapeHtml(it.jp)}</div>`:'');
...
function maskChunk(sentence,chunk){const p=splitAroundChunk(sentence,chunk);return p.found?`${escapeHtml(p.left)}<span class="targetMask">［注目箇所］</span>${escapeHtml(p.right)}`:escapeHtml(sentence)}
...
function showHint(level){
 ...
 if(level===1){
   ...
   panel.innerHTML=`<b>ヒント1｜関係を見る</b><br>注目表現を当てにいく前に、左と右がどうつながるかを見てみよう。<div class="relationFrame"><span>${escapeHtml(parts.left.trim())}</span><strong>?</strong><span>${escapeHtml(parts.right.trim())}</span></div><div class="small">「左の内容を、右がどう受けている？」と考える。</div>`;
   E('hint1Btn').disabled=true;E('hint2Btn').classList.remove('hidden');
 }else{
   panel.innerHTML=`<b>ヒント2｜注目表現を見る</b><br>関係を考えたあとで、文章を動かす表現を確認しよう。`;
   E('sentence').innerHTML=highlight(it.sentence,it.chunk)+(it.jp?`<div class="jp">${escapeHtml(it.jp)}</div>`:'');
...
function mastery(ok,t){let s=t<=12?42:t<=20?31:t<=30?18:t<=45?7:0;return ok?Math.min(100,44+s):Math.max(0,18+Math.round(s*.2)-(t<=20?12:0))}
function answer(ans,btn){if(answerLocked)return;answerLocked=true;let it=currentPack[currentIndex],t=elapsed(),ok=ans===it.type,m=mastery(ok,t),before=pointTotal(),rule=pointRule(before),delta=activeProfile==='learner'?(ok?rule.reward:-rule.penalty):0,total=Math.max(0,before+delta);delta=total-before;[...E('choices').children].forEach(b=>{b.disabled=true;if(b.textContent.includes(it.type))b.classList.add('correct')});btn.classList.add(ok?'correct':'wrong');let log={ts:new Date().toISOString(),session_id:currentSessionId||newSessionId(currentWeekId||'single'),item_index:currentIndex+1,item_id:it.id,type:it.type,theme:it.theme,title:it.title,chunk:it.chunk,answer:ans,correct_answer:it.type,ok,time_sec:t,timeout:t>limit,mastery_score:m,hint_level:hintLevel,hint_events:hintTimes,sentence:it.sentence,jp:it.jp,explanation:it.reveal,aha:it.aha,feedback:ok?`この読み方でOKです。${hintLevel?`ヒント${hintLevel}まで使って到達しました。次は一段少ないヒントで挑戦してみよう。`:`前後の関係から自力で到達できました。`}`:`今回は「${label(it.type)}」が一番自然です。まず前後の関係を見て、最後に「${it.chunk}」を確認しよう。`,point_delta:delta,point_total:total,point_level:rule.level,learner_name:currentLearnerName(),learner_id:currentLearnerId(),user_profile:activeProfile,app_mode:activeProfile==='learner'?'learning':'test',app_version:APP_VERSION,question_bank_version:QUESTION_BANK_VERSION,week_id:currentWeekId,device_info:getDeviceInfo()};let d=CLESStorage.load();CLESStorage.save({logs:[...(d.logs||[]),log],state:d.state||{}});E('sentence').innerHTML=highlight(it.sentence,it.chunk)+(it.jp?`<div class="jp">${escapeHtml(it.jp)}</div>`:'');updatePointUI();showFeedback(log)}
function label(id){return (groups.find(g=>g.id===id)||{}).label||id}function showFeedback(l){let f=E('feedback');f.classList.remove('hidden');f.className='feedback '+(l.ok?'ok':'ng');let s=l.ok&&l.time_sec<=12?'12秒以内で正解。構造がかなり速く見えています。':!l.ok&&l.time_sec<=20?'速く判断しています。次は根拠を1つ確認しましょう。':l.ok&&l.time_sec>=30?'正解です。次は30秒以内を目指せます。':'正誤だけでなく、どこを見たかを確認しましょう。',delta=Number(l.point_delta||0),deltaClass=delta>0?'pointPlus':delta<0?'pointMinus':'pointZero',deltaText=delta>0?`＋${delta}`:delta<0?`${delta}`:'±0';f.innerHTML=`<b>${l.ok?'正解':'惜しい'}</b><span class="pointDelta ${deltaClass}">${deltaText} pt</span><br>${l.feedback}<br><br><b>解説：</b>${escapeHtml(l.explanation||'')}<br><b>見抜き方：</b>${escapeHtml(l.aha||'')}<br><br>${s}<br><b>現在 ${Number(l.point_total||pointTotal())} pt</b>`}
...
function finishSession(){clearInterval(timer);showScreen('reviewScreen');if(activeProfile==='learner'){renderReview(learnerLogs().filter(x=>x.session_id===currentSessionId))}else{E('reviewText').innerHTML='<b>管理・確認モードでの実行です。</b><br><br>この結果は学習者の理解度・ポイントには反映されません。';E('reviewWarning').innerHTML='設定から学習者モードへ切り替えると、学習レビューを開始できます。';E('reviewNextActions').classList.add('hidden')}}
...
function renderProgress(){if(!E('statsGrid')||!E('functionProgress'))return;let l=learnerLogs(),a=aggregate(l),pts=pointTotal(),rule=pointRule(pts);E('statsGrid').innerHTML=[['ポイント',pts+' pt'],['レベル','LV '+rule.level],['問題数',a.n],['正答率',a.accuracy+'%'],['理解度',a.mastery+'%'],['平均時間',a.avg+'s']].map(x=>`<div class="stat"><b>${x[1]}</b><span>${x[0]}</span></div>`).join('');let p=E('functionProgress');p.innerHTML='';groups.forEach(g=>{let q=l.filter(x=>(x.correct_answer||x.type)===g.id),z=aggregate(q),sc=q.length?Math.round((z.accuracy+z.mastery)/2):0;p.innerHTML+=`<div class="progressRow"><div class="progressRowTop"><span>${g.label} / ${g.id}</span><span>${sc}%</span></div><div class="meter"><div class="fill" style="width:${sc}%"></div></div></div>`})}
function renderReview(src){if(!E('reviewText')||!E('reviewWarning'))return;let l=src&&src.length?src:learnerLogs().slice(-10),a=aggregate(l),by={};groups.forEach(g=>{let q=l.filter(x=>(x.correct_answer||x.type)===g.id);if(q.length)by[g.id]=aggregate(q)});let weak=Object.entries(by).sort((x,y)=>(x[1].accuracy+x[1].mastery)-(y[1].accuracy+y[1].mastery))[0],strong=Object.entries(by).sort((x,y)=>(y[1].accuracy+y[1].mastery)-(x[1].accuracy+x[1].mastery))[0],pts=pointTotal(),rule=pointRule(pts);if(!l.length){E('reviewText').innerHTML='まだ学習ログがありません。理一チャレンジを10問だけ試してみましょう。';E('reviewWarning').innerHTML='管理・確認モードのログは学習レビューに入りません。';E('reviewNextActions').classList.add('hidden');return}else{E('reviewNextActions').classList.remove('hidden')}E('reviewText').innerHTML=`<b>直近${l.length}問のレビュー</b><br><br>獲得ポイント：${l.reduce((s,x)=>s+Number(x.point_delta||0),0)} pt<br>合計：${pts} pt（LEVEL ${rule.level}）<br>正答率：${a.accuracy}%<br>理解度：${a.mastery}%<br>平均回答時間：${a.avg}秒<br><br>${strong?`得意：${label(strong[0])}<br>`:''}${weak?`次に見るポイント：${label(weak[0])}<br><br>`:''}${a.fastWrong?'速く間違えた問題があります。根拠を一つ確認しましょう。':'速さと正確さのバランスを確認しましょう。'}`;E('reviewWarning').innerHTML=`<b>次の10問への反映</b><br>${weak?label(weak[0])+'を少し多めにします。':''} 同じ型や注目表現ばかりにはしません。`}
...
function archiveLogs(){
 try{
   ...
   const archive={
     schema:'jcles-learning-archive',
     schema_version:'1.0',
     archived_at:new Date().toISOString(),
     archive_name:(period||'J-CLES Archive').trim(),
     app_version:APP_VERSION,
     question_bank_version:QUESTION_BANK_VERSION,
     summary:buildArchiveSummary(all),
     logs:all
   };
   const safeName=(archive.archive_name||'jcles_archive').replace(/[\\/:*?"<>|]/g,'_');
...
function showLogStats(){
 try{
   ...
   const preferredOrder=groups.map(g=>g.id);
   const rows=Object.entries(byType).sort(([a],[b])=>{
     ...
     const group=groups.find(g=>g.id===k);
     const groupLabel=group&&group.label&&group.label!==k?group.label:'';
     return `<tr><td class="statsFunction"><b>${escapeHtml(k)}</b>${groupLabel?`<span>${escapeHtml(groupLabel)}</span>`:''}</td><td>${v.n}問</td><td><span class="scorePill ${scoreClass(acc)}">${acc}%</span></td><td>${mt}秒</td><td><span class="scorePill ${scoreClass(mm)}">${mm}%</span></td></tr>`;
   }).join('');
   E('logStatsSummary').innerHTML=`<div><b>${learner.length}</b><span>${escapeHtml(currentLearnerName())}の学習ログ</span></div><div><b>${test.length}</b><span>管理・確認ログ</span></div><div><b>${pointTotal()}</b><span>現在ポイント</span></div>`;
   E('logStatsContent').innerHTML=rows?`<div class="statsTableWrap"><table class="statsTable"><thead><tr><th>Function</th><th>問題数</th><th>正答率</th><th>平均時間</th><th>理解度</th></tr></thead><tbody>${rows}</tbody></table></div>`:'<div class="statsEmpty">Function別ログはまだありません。</div>';
   E('logStatsModal').classList.add('show');
   E('closeLogStats').focus();
 }catch(err){console.error(err);alert('ログ統計を表示できませんでした。');}
}
function scoreClass(value){return value>=80?'scoreGood':value>=60?'scoreCaution':'scoreWeak'}
function escapeHtml(value){return String(value).replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]))}
function closeLogStats(){const modal=E('logStatsModal');if(modal)modal.classList.remove('show')}
function updateLogStatus(){
 ...
 try{
   ...
   const learner=learnerLogs();
   const test=all.filter(l=>l.user_profile==='developer'||l.app_mode==='test'||l.mode==='test');
   el.textContent=`${currentLearnerName()}：学習${learner.length}件 / ${pointTotal()}pt / 管理・確認${test.length}件`;
 }catch(err){
   console.error(err);
...
function backupLogs(){
 try{
   ...
   downloadFile('jcles_user_data_backup_'+new Date().toISOString().slice(0,10)+'.json',JSON.stringify(bundle,null,2));
 }catch(err){
   console.error(err);
...
async function restoreLogs(ev){
 ...
 try{
   ...
   console.error(err);
   alert('復元できませんでした。J-CLESのJSONバックアップを選択してください。');
 }finally{
   ev.target.value='';
 }
}
function clearLogsByMode(mode){
 ...
 const message=isLearning
   ?`${currentLearnerName()}の学習ログとポイントを初期化します。元に戻せません。先にバックアップまたはアーカイブしてください。`
   :'管理・確認モードのログだけを消去します。';
 if(!confirm(message))return;
 if(isLearning){
   const word=prompt(`${currentLearnerName()}の学習ログを本当に初期化する場合は DELETE と入力してください。`);
 ...
 try{
   ...
   const kept=(data.logs||[]).filter(l=>{
     ...
     const sameLearner=!test&&(!l.learner_id||l.learner_id===currentLearnerId());
     return isLearning?!sameLearner:!test;
   });
...
function getDeviceId(){let id=localStorage.getItem('jcles.deviceId.v1');if(!id){id='dev_'+Math.random().toString(36).slice(2)+Date.now().toString(36);localStorage.setItem('jcles.deviceId.v1',id)}return id}boot().catch(err=>{
 console.error(err);
 renderTodayFallback();
 updateLogStatus();
});
